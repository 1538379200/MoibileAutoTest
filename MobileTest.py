...
def gotXml(file):
    ...
    subprocess.call(f'adb shell rm /sdcard/{file}.xml', stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT, shell=True)
    ...
    try:
        with open(file + '.xml', 'rb') as f:
            ...
            for i in range(0, len(boundsText)):  # 循环所有的标签，找出定位属性的值，加入文本列表中
                ...
                XmlTextFilter = re.findall(r'[\u4e00-\u9fa50-9A-Za-z]+',XmlText)
                # print(XmlTextFilter)
                # if XmlText:
                #     if '"' in XmlText:
                #         CanChoiceTextList.append(XmlText.replace('"', ''))
                #     else:
                #         CanChoiceTextList.append(XmlText)
                # else:
                #     pass
                # print(XmlTextFilter)
                if XmlTextFilter == []:
                    ...
                    CanChoiceTextList.append(XmlTextFilter[0])
            ...
            for textel in CanChoiceTextList:
                sg.OneLineProgressMeter('查找进度：', barNum, maxBar - 1, bar_color='#18A058', button_color='#18A058')
                ...
                try:
                    ...
                    if soup.select(f'node[{type_}~="{textel}"]')[nums]['clickable'] == 'true':  # 当元素可点击时
                        ...
                        XmlAxis = soup.select(f'node[{type_}~="{textel}"]')[nums].parent['bounds']
...
layout = [
    ...
    [sg.Text('请选择操作：', background_color='#ffffff'),
     sg.DropDown(values=['启动App', '点击坐标', '滑动屏幕', '输入文字(带ENTER)', '输入文字(不带ENTER)', '等待', '长按', '智能等待'],
                 default_value='滑动屏幕', size=20,
    ...
    [sg.Text('1、在此输入文字。2、启动App将包名和activity放此处，/分隔，形如：com.package.name'
             '/com.package.name.MainActivity。3、等待直接写入数字(秒)。4、长按不写时间默认2秒：', background_color='#ffffff')],
    [sg.Input(expand_x=True, do_not_clear=False, tooltip='目前仅支持英文输入，\n使用中文请搜索adb中文输入法')],  # values1
    [sg.T('此为自动获取当前页面的可点击元素，并转换为坐标值，常用 text content-desc ,建议 text,点击将弹出类型输入，输入如：text', background_color='#ffffff'),
     sg.Button('获取页面可点击坐标', expand_x=True, button_color='#18A058', font=('微软雅黑', 9))],
    [sg.Output(size=(120, 21), font=('微软雅黑', 12), key='OutPut', echo_stdout_stderr=True)],
        ...
        try:
            ...
            if devicesNameFilter:
                ...
                for i in list_:
                    ...
                    sg.OneLineProgressMeter('测试进度：', num, len(list_), bar_color='#18A058', button_color='#18A058')
                    num += 1
                print('*-' * 27, '测试完成', '-*' * 27, end='\n\n')
                sg.popup('测试完成!', title='提示', line_width=1000, background_color='#ffffff',
                         button_color=('#ffffff', '#18A058'), icon='GuiIcon.ico')
        ...
        try:
            for i in range(0, len(fil1) - 1):  # 循环切割的列表
                ...
                try:
                    ...
                    pass
            list_ = putList  # 将putlist的数据赋值给真正保存测试队列的list_
        ...
        if devicesNameFilter:
            ...
            if AxisType:  # 当用户输入了定位方式，则运行下面的代码，否则视为取消操作
                ...
                window2 = sg.Window('页面可点击元素', layout2, background_color='#ffffff', icon='GuiIcon.ico', resizable=True,
                                    finalize=True)
                sg.popup_auto_close('正在搜集数据源……', title='提示', background_color='#ffffff', auto_close_duration=3,
                                    button_color='#18A058', icon='GuiIcon.ico')
                gotXml('elementMobile')  # 运行获取手机页面xml元素的脚本
                axisEl = checkXml(AxisType, 'elementMobile')  # 运行筛选页面元素定位的方法，获取一个字典，包含元素和坐标
                # axisString = ''
                if axisEl == 'null':
                    sg.popup('元素查找出错，或者没有元素',title='提示', background_color='#ffffff', button_color=('#ffffff', '#18A058'),
                    icon='GuiIcon.ico')
                elif len(axisEl.values()) == 0:
